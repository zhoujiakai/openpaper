...
> - LeetCode热题100（100道题）：https://leetcode.cn/studyplan/top-100-liked/
...
### ==108题，#二叉树，#二叉搜索树，将有序树组转换成二叉搜索树==

20260225周三

不会做。

98题，#二叉树，#二叉搜索树，#深度优先搜索，验证二叉搜索树
...
230题，#二叉树，#二叉搜索树，#深度优先搜索，二叉搜索树中第K小的元素
...
34题，#二分查找，#数组，在排序数组中查找元素的第一个和最后一个位置
...
- 遇到字母：摘出子串
- 遇到数字：计算子串需要重复多少次
- 遇到左括号：进入递归；递归的退出条件是指针指到字符串末尾，或者遇到右括号
- 遇到右括号：直接返回结果（因为遇到左括号就直接进递归了，所以有括号只会在递归内部遇到）
- 解决完一个子序列之后，继续递归解决后续串
...
class Solution:
    def decodeString(self, s: str) -> str:
        def get_ans(s, start):
            ...
            # 以字母开头
            if 'a' <= s[start] <= 'z':
                while start < len(s) and 'a' <= s[start] <= 'z':
                    ans += s[start]
                    start += 1
            # 以数字开头
            if start < len(s) and '0' <= s[start] <= '9':
                cnt = 0
                while start < len(s) and s[start] != '[':
                    cnt = cnt * 10 + (ord(s[start]) - ord('0'))
                    start += 1
            # 以左括号开头：开始递推查找内嵌子串
            if start < len(s) and s[start] == '[':
                start += 1
                subs, start = get_ans(s, start)
                ans += subs * cnt
            # 以右括号开头
            if start < len(s) and s[start] == ']':
                return ans, start+1
            # 继续查找后续串
            next_ans, start = get_ans(s, start)
            return ans + next_ans, start
        ans, end = get_ans(s, 0)
        return ans
```
