...
## Q6：基于寄存器的方式控制 STM32 最小系统板上的测试灯亮需要哪几个步骤

> 日期：20260617周三

STM32F103C8T6 最小系统板上的测试灯接在 **PC13** 口，低电平点亮。基于寄存器方式只需 3 步：
...
- RCC 是复位和时钟控制器，GPIO 外设默认关闭（省电），需要先开时钟。
- APB2 总线连接 GPIOC，对应使能位是 bit 4（IOPCEN）。
- `0x00000010` = 二进制 `...0001 0000`，bit4 置 1，开启 GPIOC 时钟。
- 不开这行，后面操作 GPIOC 的寄存器全部无效。

**步骤 2：配置 PC13 为推挽输出模式**

```c
GPIOC->CRH = 0x00300000;
```

- CRH 是端口配置高寄存器，负责 GPIO Pin 8~15。
- PC13 对应 CRH 中高 4 位（bit 23~20）。
- 每个引脚用 4 位配置：低 2 位 MODE（模式），高 2 位 CNF（配置）。
- `0011` = MODE=`11`（输出模式，最大速度 50MHz）+ CNF=`00`（通用推挽输出）。
- `0x00300000` 只修改 PC13 对应的 4 个 bit。

**步骤 3：控制 PC13 输出低电平点亮 LED**

```c
GPIOC->ODR = 0x00000000;
```

- ODR 是端口输出数据寄存器，写 0 输出低电平，写 1 输出高电平。
- `0x00000000` 把所有引脚拉低（包括 PC13 输出低电平）。
- 测试灯硬件是低电平点亮（LED 阳极接 3.3V，阴极经限流电阻接 PC13），输出 0 = 灯亮。
